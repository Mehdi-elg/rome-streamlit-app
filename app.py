...
with tab_all:
    ...
    for key, default in [
        ...
        ("start_clicked",       False),  # Nouveau flag
    ]:
    ...
    with col1:
        st.button("🚀 Lancer l'extraction complète", 
                 key="btn_start_full",
    ...
    # Si on vient de cliquer sur START, on récupère d'abord les codes
    if st.session_state.start_clicked and not st.session_state.all_codes:
        st.session_state.start_clicked = False  # Réinitialise le flag
        try:
            ...
            st.session_state.df_all_fipu = pd.DataFrame()

    # Traitement des métiers (un seul à la fois par cycle)
    if st.session_state.extraction_en_cours and st.session_state.all_codes:
        ...
        if done_so_far < total and not st.session_state.stop_full:
            status_text.info(f"🔄 Traitement en cours… {done_so_far + 1} / {total}")
            
            # Traite UN SEUL code à la fois
            ...
            # Force le rafraîchissement pour traiter le prochain code
            # et permettre au bouton STOP d'être cliqué entre chaque traitement
            import time
            time.sleep(0.1)  # Petit délai pour éviter de surcharger
            st.rerun()
        
        elif st.session_state.stop_full or done_so_far >= total:
            # Extraction terminée ou stoppée
            st.session_state.extraction_en_cours = False
            ...
            if st.session_state.extraction_done:
                status_text.success(f"✅ Extraction terminée — {nb_ok} métiers récupérés, {nb_err} erreurs.")
            else:
                status_text.warning(f"⛔ Extraction arrêtée — {nb_ok} métiers récupérés, {nb_err} erreurs.")
    ...
    if df_all_fipu is not None and not df_all_fipu.empty:
    ...
    elif df_all_fipu is not None and df_all_fipu.empty:
